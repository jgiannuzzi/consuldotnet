...
def main():
    doc = yaml.safe_load(open('matrix.yml'))
    
    matrix = load_matrix(doc['matrix'])
    ...
    for op in doc['operations']:
        ...
        if op['type'] == 'merge':
            ...
            intersection = matrix.columns.intersection(op_matrix.columns).difference(op['match'])
            matrix = matrix.append(matrix.drop(intersection, axis=1).merge(op_matrix, on=op['match'])).drop_duplicates()
        else:
